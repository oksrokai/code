                   ...
                   unsigned char *sk)
{
  ...
  for(i=0;i<OKAI_K;i++){
    ...
    poly_reduce(&pkp);
    poly_compress_10(pk + i * OKAI_COMPRESSEDPKBYTES, pkp.coeffs);
  }
               ...
               const unsigned char *coins)
{
  ...
  poly_decompress_10(pkp->coeffs, pk); 
  poly_ntt(pkp);
  poly_basemul(v, pkp, &sp.vec[0]);
  for(i=1;i< OKAI_K;i++){
    poly_decompress_10(pkp->coeffs, pk + i*OKAI_COMPRESSEDPKBYTES);
    poly_ntt(pkp);
  ...
  poly_getnoise(&epp,coins,nonce++);
  poly_add(v, v, &epp);
  poly_con_3(c+OKAI_POLYVECCOMPRESSEDCBYTES,m,v->coeffs);
}
               ...
               const unsigned char *sk)
{
  ...
  // mp = v - mp
  poly_invntt(&mp);
  poly_rec_3(m,c+OKAI_POLYVECCOMPRESSEDCBYTES,mp.coeffs);
}
