...
#define KEM_BENCH_ITERATIONS 100000
#define FALSE 0
#define TRUE  1
...
static void kem_bench(const int iterations)
{
    uint8_t pk[CRYPTO_PUBLICKEYBYTES];
    uint8_t sk[CRYPTO_SECRETKEYBYTES];
    uint8_t ss_encap[CRYPTO_BYTES], ss_decap[CRYPTO_BYTES];
    ...
    TIME_OPERATION_ITERATIONS({ crypto_kem_keypair(pk, sk); }, "Key generation", iterations);
    crypto_kem_keypair(pk, sk);

    TIME_OPERATION_ITERATIONS({ crypto_kem_enc(ct, ss_encap, pk); }, "KEM encapsulate", iterations);
    crypto_kem_enc(ct, ss_encap, pk);

    TIME_OPERATION_ITERATIONS({ crypto_kem_dec(ss_decap, ct, sk); }, "KEM decapsulate", iterations);
    crypto_kem_dec(ss_decap, ct, sk);
}
...
int main()
{
    ...
    PRINT_TIMER_HEADER
    kem_bench(KEM_BENCH_ITERATIONS);
    PRINT_TIMER_FOOTER
