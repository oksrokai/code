...
static void unpack_pk(polyvec *pk, unsigned char *seed, const unsigned char *packedpk)
{
  ...
  for(i=0;i<OKAI_SYMBYTES;i++)
    seed[i] = packedpk[i+OKAI_POLYVECCOMPRESSEDPKBYTES];
}
...
static void pack_ciphertext(unsigned char *r, polyvec *b, poly *v)
{
  ...
  poly_compress(r+OKAI_POLYVECCOMPRESSEDCBYTES, v);
}
...
static void unpack_ciphertext(polyvec *b, poly *v, const unsigned char *c)
{
  ...
  poly_decompress(v, c+OKAI_POLYVECCOMPRESSEDCBYTES);
}
  ...
  for(int j=0;j<OKAI_K;j++) {
    ...
    while (ctr < OKAI_N/4)
    {
      ...
      if (val < 8*OKAI_Q) {
        ...
        c[k++] = (int16_t) val;
        if (k == 4) {
          doublebasemul_asm_acc(&r->coeffs[4*ctr], &b->vec[j].coeffs[4*ctr], c, zetas[ctr+64]);
          ctr++;
          k = 0;
        }
      }

      if (pos + 2 > buflen && ctr < OKAI_Q/4) {
        off = buflen % 2;
        for(l = 0; l < off; l++)
                   ...
                   unsigned char *sk)
{
  ...
  sha3_512(buf, buf, OKAI_SYMBYTES);
  ...
  for(i=0;i<OKAI_K;i++){
    ...
    poly_reduce(&pkp);
    poly_compresspk(pk + i * OKAI_COMPRESSEDPKBYTES, &pkp); 
  }

  for(i=0;i<OKAI_SYMBYTES;i++)
    pk[i+OKAI_POLYVECCOMPRESSEDPKBYTES] = publicseed[i];
}
               ...
               const unsigned char *coins)
{
  ...
  const unsigned char *seed = pk + OKAI_POLYVECCOMPRESSEDPKBYTES;
  int i;
  unsigned char nonce=0;
  ...
  for(i=0;i<OKAI_K;i++){
    ...
    poly_reduce(&bp);
    poly_compressc(c + i * OKAI_COMPRESSEDCBYTES, &bp); 
  }


  poly_decompresspk(pkp, pk); 
  poly_ntt(pkp);
  poly_basemul(v, pkp, &sp.vec[0]);
  for(i=1;i< OKAI_K;i++){
    poly_decompresspk(pkp, pk + i*OKAI_COMPRESSEDPKBYTES);
    poly_ntt(pkp);
  ...
  // // c = bp + v
  // poly_compress(c+OKAI_POLYVECCOMPRESSEDCBYTES, v);
  poly_con_4(c+OKAI_POLYVECCOMPRESSEDCBYTES,m,v);
}

void pke_enc(unsigned char *c, const unsigned char *m, const unsigned char *pk)
{
  ...
  indcpa_enc(c, m, pk, kr+OKAI_SYMBYTES);  /* coins are in kr+OKAI_SYMBYTES */
}
               ...
               const unsigned char *sk)
{
  ...
  poly_decompressc(&bp, c);
  poly_ntt(&bp);
  poly_frombytes(&skp, sk);
  poly_basemul(&mp, &bp, &skp);
  for(int i=1;i<OKAI_K;i++){
    poly_decompressc(&bp, c + i*OKAI_COMPRESSEDCBYTES);
    poly_ntt(&bp);
    poly_frombytes(&skp, sk + i*OKAI_POLYBYTES);
    poly_basemul_acc(&mp, &bp, &skp);
  }

  // mp = v - mp
  poly_invntt(&mp);
  poly_rec_4(m,c+OKAI_POLYVECCOMPRESSEDCBYTES,&mp);
}
