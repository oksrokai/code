...
static void poly_decompress10(poly * restrict r, const uint8_t a[320+12])
{
  unsigned int i;
  __m256i f;
  const __m256i q = _mm256_set1_epi32((OSKR_Q << 16) + 4*OSKR_Q);
...
static void poly_compress11(uint8_t r[352*2+4], const poly * restrict a)
{
  unsigned int i;
  __m256i f0, f1, f2;
  __m128i t0, t1;
...
static void poly_decompress11(poly * restrict r, const uint8_t a[352*2+20])
{
  unsigned int i;
  __m256i f;
...
*                            (needs space for OSKR_POLYVECCOMPRESSEDBYTES)
...
#if (OSKR_POLYVECCOMPRESSEDBYTES == (OSKR_K * 320))
  for(i=0;i<OSKR_K;i++)
    poly_compress10(&r[320*i],&a->vec[i]);
#elif (OSKR_POLYVECCOMPRESSEDBYTES == (OSKR_K * 352 * 2))
  poly_compress11(&r[0],&a->vec[0]);
  poly_compress11(&r[352*2],&a->vec[1]);
...
*                                  (of length OSKR_POLYVECCOMPRESSEDBYTES)
**************************************************/
...
#elif (OSKR_POLYVECCOMPRESSEDBYTES == (OSKR_K * 352 * 2))
  poly_decompress11(&r->vec[0],&a[0]);
  poly_decompress11(&r->vec[1],&a[352*2]);
#endif
}
...
{
  #if (OSKR_N == 256 && OSKR_K == 2)
  poly t;
  poly_basemul_montgomery(r,&a->vec[0],&b->vec[0]);
  poly_basemul_montgomery(&t,&a->vec[1],&b->vec[1]);
  poly_add(r, r, &t);
#elif (OSKR_N == 256 && OSKR_K == 3)
  poly t,u;
  poly_basemul_montgomery(r,&a->vec[0],&b->vec[0]);
  poly_basemul_montgomery(&t,&a->vec[1],&b->vec[1]);
  poly_add(r, r, &t);
  poly_basemul_montgomery(&u,&a->vec[2],&b->vec[2]);
  poly_add(r, r, &u);
#elif (OSKR_N == 512)
  poly t;
