...
/*************************************************
* Name:        OKAI_shake128_absorb
*
* Description: Absorb step of the SHAKE128 specialized for the OKAI context.
*
* Arguments:   - uint64_t *s:                     pointer to (uninitialized) output Keccak state
*              - const unsigned char *input:      pointer to OKAI_SYMBYTES input to be absorbed into s
...
{
  unsigned char extseed[OKAI_SYMBYTES+2];
  int i;
  ...
  extseed[i++] = x;
  extseed[i]   = y;
  shake128_absorb(s->s, extseed, OKAI_SYMBYTES+2);
}

/*************************************************
* Name:        OKAI_shake128_squeezeblocks
*
...
*              - const unsigned char * key:  pointer to the key (of length OKAI_SYMBYTES)
...
{
  unsigned char extkey[OKAI_SYMBYTES+1];
  size_t i;
  ...
  shake256(output, outlen, extkey, OKAI_SYMBYTES+1);
}
